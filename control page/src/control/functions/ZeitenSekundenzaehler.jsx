...
class ZeitenSekundenzaehler extends Component {
  getVarWert(v) {
  ...
  reset() {
    if (window.location.search.includes('?')) {
      this.props.websocketSend({
        Domain: 'KO', Type: 'bef', Command: 'TimerReset', Value1: 5,
      });
    }
  }

  render() {
    const x = this.props.timer ? this.props.timer.find((y) => y.Name === 'Sekundenzähler') : null;
    const y = this.props.timerObjects ? this.props.timerObjects.find((z) => z.ID === 5) : null;
...
ZeitenSekundenzaehler.propTypes = {
...
export default ZeitenSekundenzaehler;
