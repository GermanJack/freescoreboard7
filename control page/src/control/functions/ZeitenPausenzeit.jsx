...
class ZeitenPausenzeit extends Component {
  getVarWert(v) {
  ...
  reset() {
    if (window.location.search.includes('?')) {
      this.props.websocketSend({
        Domain: 'KO', Type: 'bef', Command: 'TimerReset', Value1: 3,
      });
    }
  }

  render() {
    const x = this.props.timer ? this.props.timer.find((y) => y.Name === 'Pausenzeit') : null;
    const y = this.props.timerObjects ? this.props.timerObjects.find((z) => z.ID === 3) : null;
...
export default ZeitenPausenzeit;
